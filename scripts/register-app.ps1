# Helm365 — Automated Azure AD App Registration (PowerShell)
# Creates the app registration with all 30 required permissions and grants admin consent.
#
# Prerequisites: Azure CLI (az) installed and logged in
#   az login
#
# Usage:
#   .\scripts\register-app.ps1                    # Single-tenant
#   .\scripts\register-app.ps1 -MultiTenant       # Multi-tenant (MSP / GDAP)

param(
    [switch]$MultiTenant
)

$ErrorActionPreference = "Stop"
$AppName = "Helm365"

Write-Host ""
Write-Host "  Helm365 -- App Registration Setup" -ForegroundColor Cyan
Write-Host "  =================================" -ForegroundColor Cyan
Write-Host ""

if ($MultiTenant) {
    $Audience = "AzureADMultipleOrgs"
    Write-Host "  Mode: Multi-tenant (MSP / GDAP)" -ForegroundColor Yellow
} else {
    $Audience = "AzureADMyOrg"
    Write-Host "  Mode: Single tenant"
}

Write-Host ""
Write-Host "Step 1: Creating app registration..." -ForegroundColor Green

$AppId = az ad app create `
    --display-name $AppName `
    --sign-in-audience $Audience `
    --query appId -o tsv

Write-Host "  App (Client) ID: $AppId"

$TenantId = az account show --query tenantId -o tsv
Write-Host "  Tenant ID: $TenantId"

Write-Host ""
Write-Host "Step 2: Adding API permissions (30 permissions)..." -ForegroundColor Green

# Build permissions JSON
$PermissionsJson = @'
[{
  "resourceAppId": "00000003-0000-0000-c000-000000000000",
  "resourceAccess": [
    {"id": "df021288-bdef-4463-88db-98f22de89214", "type": "Role"},
    {"id": "741f803b-c850-494e-b5df-cde7c675a1ca", "type": "Role"},
    {"id": "50483e42-d915-4231-9639-7fdb7fd190e5", "type": "Role"},
    {"id": "5b567255-7703-4780-807c-7be8301ae99b", "type": "Role"},
    {"id": "dbaae8cf-10b5-4b86-a4a1-f871c94c6695", "type": "Role"},
    {"id": "7ab1d382-f21e-4acd-a863-ba3e13f7da61", "type": "Role"},
    {"id": "498476ce-e0fe-48b0-b801-37ba7e2685c6", "type": "Role"},
    {"id": "810c84a8-4a9e-49e6-bf7d-12d183f40d01", "type": "Role"},
    {"id": "e2a3a72e-5f79-4c64-b1b1-878b674786c9", "type": "Role"},
    {"id": "40f97065-369a-49f4-947c-6a255697ae91", "type": "Role"},
    {"id": "6931bccd-447a-43d1-b442-00a195474933", "type": "Role"},
    {"id": "246dd0d5-5bd0-4def-940b-0421030a5b68", "type": "Role"},
    {"id": "01c0a623-fc9b-48e9-b794-0756f8e8f067", "type": "Role"},
    {"id": "bf394140-e372-4bf9-a898-299cfc7564e5", "type": "Role"},
    {"id": "472e4a4d-bb4a-4026-98d1-0b0d74cb74a5", "type": "Role"},
    {"id": "b0afded3-3588-46d8-8b3d-9842eff778da", "type": "Role"},
    {"id": "dc5007c0-2d7d-4c42-879c-2dab87571379", "type": "Role"},
    {"id": "656f6061-f9fe-4807-9708-6a2e0934df76", "type": "Role"},
    {"id": "8e8e4742-1d95-4f68-9d56-6ee75648c72a", "type": "Role"},
    {"id": "230c1aed-a721-4c5d-9cb4-a90514e508ef", "type": "Role"},
    {"id": "86632667-cd15-4845-ad89-48a88e8412e1", "type": "Role"},
    {"id": "d72bdbf4-a59b-405c-8b04-5995895819ac", "type": "Role"},
    {"id": "926a6798-b100-4a20-a22f-a4918f13951d", "type": "Role"},
    {"id": "2f51be20-0bb4-4fed-bf7b-db946066c75e", "type": "Role"},
    {"id": "243333ab-4d21-40cb-a475-36241daa0842", "type": "Role"},
    {"id": "5b07b0dd-2377-4e44-a38d-703f09a0dc3c", "type": "Role"},
    {"id": "dc377aa6-52d8-4e23-b271-2a7ae04cedf3", "type": "Role"},
    {"id": "9241abd9-d0e6-425a-bd4f-47ba86e767a4", "type": "Role"},
    {"id": "7a6ee1e7-141e-4cec-ae74-d9db155731ff", "type": "Role"},
    {"id": "78145de6-330d-4800-a6ce-494ff2d33d07", "type": "Role"}
  ]
}]
'@

$Permissions = @(
    "User.Read.All",
    "User.ReadWrite.All",
    "UserAuthenticationMethod.ReadWrite.All",
    "Group.Read.All",
    "GroupMember.ReadWrite.All",
    "Directory.Read.All",
    "Organization.Read.All",
    "Mail.Read",
    "Mail.ReadWrite",
    "MailboxSettings.Read",
    "MailboxSettings.ReadWrite",
    "Policy.Read.All",
    "Policy.ReadWrite.ConditionalAccess",
    "SecurityEvents.Read.All",
    "SecurityAlert.Read.All",
    "AuditLog.Read.All",
    "IdentityRiskyUser.Read.All",
    "IdentityRiskyUser.ReadWrite.All",
    "DelegatedPermissionGrant.ReadWrite.All",
    "Reports.Read.All",
    "ThreatSubmission.Read.All",
    "ThreatSubmission.ReadWrite.All",
    "ThreatPolicy.ReadWrite.All",
    "DeviceManagementManagedDevices.Read.All",
    "DeviceManagementManagedDevices.ReadWrite.All",
    "DeviceManagementManagedDevices.PrivilegedOperations.All",
    "DeviceManagementConfiguration.Read.All",
    "DeviceManagementConfiguration.ReadWrite.All",
    "DeviceManagementApps.Read.All",
    "DeviceManagementApps.ReadWrite.All"
)

foreach ($p in $Permissions) {
    Write-Host "  + $p"
}

$TempFile = [System.IO.Path]::GetTempFileName()
$PermissionsJson | Out-File -FilePath $TempFile -Encoding utf8

az ad app update --id $AppId --required-resource-accesses "@$TempFile"
Remove-Item $TempFile

Write-Host ""
Write-Host "Step 3: Creating service principal..." -ForegroundColor Green
try {
    az ad sp create --id $AppId | Out-Null
} catch {
    Write-Host "  (already exists)"
}

Write-Host ""
Write-Host "Step 4: Granting admin consent..." -ForegroundColor Green
Start-Sleep -Seconds 3
try {
    az ad app permission admin-consent --id $AppId 2>$null
} catch {
    Write-Host "  Warning: Admin consent may need to be granted manually in the Azure portal." -ForegroundColor Yellow
}

Write-Host ""
Write-Host "Step 5: Creating client secret (1 year expiry)..." -ForegroundColor Green
$Secret = az ad app credential reset `
    --id $AppId `
    --display-name "Helm365 Secret" `
    --years 1 `
    --query password -o tsv

Write-Host ""
Write-Host "  =================================" -ForegroundColor Green
Write-Host "  App Registration Complete" -ForegroundColor Green
Write-Host "  =================================" -ForegroundColor Green
Write-Host ""
Write-Host "  Tenant ID:     $TenantId" -ForegroundColor White
Write-Host "  Client ID:     $AppId" -ForegroundColor White
Write-Host "  Client Secret: $Secret" -ForegroundColor White
Write-Host ""
Write-Host "  Paste these into Helm365 > Tenants > Connect Tenant."
Write-Host "  Permissions: 30 (admin consent granted)"
Write-Host ""
